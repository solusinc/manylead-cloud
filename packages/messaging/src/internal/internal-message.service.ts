...
import {
  getEventPublisher,
  getCrossOrgMirrorService,
} from "@manylead/core-services";
import type {
  EventPublisher,
 ...
 *
 * Responsável por todas as operações de mensagens internas:
...
// Singleton instance
let internalMessageServiceInstance: InternalMessageService | null = null;

/**
 * Get or create InternalMessageService singleton
 */
export function getInternalMessageService(config: MessageServiceConfig): InternalMessageService {
  internalMessageServiceInstance ??= new InternalMessageService(config);
  return internalMessageServiceInstance;
}
